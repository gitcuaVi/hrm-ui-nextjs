...
import AddAdminDrawer from 'src/views/apps/category/accesscontrol/formadmins/FormAddAdmin'
import EditAdmin from 'src/views/apps/category/accesscontrol/formadmins/FormAdmin'
...
const RowOptions = ({ id, userData }: { id: number | string; userData: UsersType }) => {
  ...
  return (
    <>
      ...
      <EditAdmin open={openEdit} setOpen={setOpenEdit} userData={userData} />
    </>
  )
}
...
const AdminList = ({ apiData }: InferGetStaticPropsType<typeof getStaticProps>) => {
  // ** State
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => (
        <CustomChip
          skin='light'
          size='small'
          label={t(row.status)} // 🆕 Dịch trạng thái "Active", "Pending", "Inactive"
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => <RowOptions id={row.id} userData={row} />
    }
  ]
